...
d3.json("json/list.json", function (error, myData) {
    ...
    // create another axis function so we can add the X Axis
    var xAxisScale = d3.scale.ordinal()
        .domain(myData.map(function (d) { // The .map function creates a new array out of just the names.
            return d.username;
        }))
    ...
    // create our new axis using the new scale
    var xAxis = d3.svg.axis()
        .scale(xAxisScale) // give it scale
        .orient("bottom") // align it at the bottom orientation
        .ticks(myData.length); // set it tothe length of the num of users
    ...
    canvas.append("g")
        .attr("class", "axis") // style y axis vis css
